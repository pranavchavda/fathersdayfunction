...
|---|---|---|---|
| `bundler` | `cart.transform.run` | product metafields `custom.bundle_product_ids` (list.variant_reference) + optional `custom.bundle_product_quantities` (list.number_integer, index-aligned) + optional `custom.bundle_choice_ids` (list.variant_reference — shopper picks ONE, recorded by the storefront in the hidden line attribute `_bundle_choice`; off-list values incl. `declined` are ignored). Expands the line into parent (own price) + gifts (+ choice) at **$0** via `fixedPricePerUnit`. Route `app/bundles` bulk-applies the metafields and tags products `bundle-parent`. | Free-gift-with-machine promos (e.g. 2× Essential Espresso), Jura accessory bundles. Not the "Complete Your Setup" add-ons — those are `custom.bundle_products` handled by the *combodiscounts* app. |
...
- Pages: `app.create-discount` (Tiered Sale), `app.createdynamiccombo` (Dynamic Combo), `app.create-combo` (Combo Builder tiers), `app.tiered-coupon`, `app.capped-discount`, `app.discount-config` (Coupon Value Limit), `app.bundles` (Bundles & free gifts: resource pickers → `app/lib/bundles.server.js` writes the three `custom.bundle_*` metafields and the `bundle-parent` tag; the list is `products(query:"tag:bundle-parent")` because Admin product search cannot filter on metafields).
